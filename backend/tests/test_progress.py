...
def test_progress_separates_local_catalog_coverage_from_production_data() -> None:
    ...
    assert config_review.status == "completed"
    assert "268 块" in config_review.description
    assert "缺少可靠依据的型号不猜测" in config_review.description
...
def test_progress_dashboard_renders_summary_and_phases() -> None:
    ...
    assert "DeepSeek动态配置调整" in response.text
