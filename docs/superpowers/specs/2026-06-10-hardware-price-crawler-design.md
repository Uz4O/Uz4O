...
Build a local, manually operated Taobao/JD price collection tool for CPU, GPU, and motherboard reference prices. The tool must remain independent from the SwiftUI app and must never overwrite production price data automatically.
...
- Search Taobao or JD one hardware model at a time and collect the first result page.
...
The crawler does not support automatic CAPTCHA handling, proxy rotation, fingerprint spoofing, background server deployment, or automatic publishing into the iOS app.
...
- `scraper.py` owns visible-browser Taobao/JD search and raw DOM extraction.
...
Browser collection and price calculation are separate. If a platform blocks a run, previously collected raw data can still be processed and reviewed.
...
- Delay between searches defaults to 8 seconds.
- Verification, login, and slider prompts are handled manually by the operator.
- The crawler does not try to bypass anti-bot controls.
...
- A dry-run CLI command processes fixture data without opening Taobao or JD.
